...
Return the first `num` rows of a SparkDataFrame as a R data.frame. If
`num` is not specified, then head() returns the first 6 rows as with R
data.frame.
...
`getNumPartitions()`, `group_by()`, `hint()`, `histogram()`,
